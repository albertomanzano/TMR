...
noise_factor = 10; %one of noise_factor pixel is corrupted
% Con 10 masl
...
for k = 1:length(dirSnaps)
    ...
    %create mask
    linearIndices = randperm(numel(m_red), round(numel(m_red)/noise_factor));
...
for k = 1:length(dirSnaps)
    ...
    snaps_cor(:, :, :, k) = (cat(3, snaps_r_cor(:,:,k), snaps_g_cor(:,:,k), snaps_b_cor(:,:,k)));
    imshow(uint8(snaps_rec(:, :, :, k)))
    title('Reconstrucción')
    drawnow;
end
%%
%save reconstructed video snaps, uncoment to storage snaps in current directory


foldername = 'repairedB';
for k=1:length(dirSnaps)
    filename = fullfile(foldername, sprintf('%d.jpg',k));
    imwrite(uint8(snaps_rec(:, :, :, k)), filename);
end
%%
%record reconstructed video
video = VideoWriter('repairedB/video.avi'); %create the video object
open(video); %open the file for writing
for k=1:length(dirSnaps) %where N is the number of images
    I = imread(fullfile(foldername, sprintf('%d.jpg',k))); %read the next image
...
for k=1:length(dirSnaps)
    filename = fullfile(foldername, sprintf('%d.jpg',k));
    imwrite(uint8(snaps_cor(:, :, :, k)), filename);
end
%%
%record reconstructed video
video = VideoWriter('./errorB/video.avi'); %create the video object
open(video); %open the file for writing
for k=1:length(dirSnaps) %where N is the number of images
    I = imread(fullfile(foldername, sprintf('%d.jpg',k))); %read the next image
    writeVideo(video,I); %write the image to file
...
%%
%show one snapsot
snap = 29; %this snap is interesting
 figure(2)
    subplot(1,3,1)
    imshow(uint8(snaps(:, :, :, snap)))
    title('Original')
    %     drawnow;
    subplot(1,3,2)
    imshow(uint8(snaps_cor(:, :, :, snap)))
    title('Píxeles corruptos')
    %     drawnow;
    subplot(1,3,3)
    imshow(uint8(snaps_rec(:, :, :, snap)))
    title('Reconstrucción')